...
def run_mcp_posting(item: Item, *, max_steps: int = 80) -> str:
    ...
    prompt = (
        ...
        "MANDATORY photo upload — do NOT skip or submit without completing this:\n"
        "The listing form has a photo upload section (look for a camera icon, a "
        "'Fotos hinzufügen' label, or a dashed upload area).\n"
        "1. Scroll to that section if needed.\n"
        "2. Click the upload area/button — this opens a native file chooser.\n"
        "3. Immediately call browser_file_upload with paths: "
        f"{json.dumps(photo_paths, ensure_ascii=True)}\n"
        "4. Wait for the thumbnails to appear before proceeding.\n"
        "5. If the upload area is not visible, take a snapshot and scroll down to find it.\n"

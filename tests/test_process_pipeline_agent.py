...
from schnapplist.agents.process_pipeline import ProcessWorkflow
...
def test_pipeline_uses_agent_on_success(tmp_path):
    ...
    with (
        patch("schnapplist.agents.process_pipeline.group_photos_by_item", return_value=[[photos_dir / "item.jpg"]]),
        patch("schnapplist.agents.process_pipeline.filter_redundant_photos", return_value=[photos_dir / "item.jpg"]),
        patch("schnapplist.agents.process_pipeline.enhance_photo", return_value=photos_dir / "item.jpg"),
        patch("schnapplist.agents.process_pipeline.run_item_research_agent", return_value=_make_agent_result(mock_output)),
        patch("schnapplist.agents.process_pipeline.write_item_report"),
    ):
...
def test_pipeline_calls_decision_callback_on_agent_failure(tmp_path):
    ...
    with (
        ...
        patch("schnapplist.agents.process_pipeline.filter_redundant_photos", return_value=[photos_dir / "item.jpg"]),
        patch("schnapplist.agents.process_pipeline.enhance_photo", return_value=photos_dir / "item.jpg"),
        patch("schnapplist.agents.process_pipeline.run_item_research_agent", side_effect=RuntimeError("LLM error")),
        patch("schnapplist.agents.process_pipeline.write_item_report"),
    ):
...
def test_pipeline_emits_item_usage(tmp_path):
    ...
    with (
        ...
        patch("schnapplist.agents.process_pipeline.filter_redundant_photos", return_value=[photos_dir / "item.jpg"]),
        patch("schnapplist.agents.process_pipeline.enhance_photo", return_value=photos_dir / "item.jpg"),
        patch("schnapplist.agents.process_pipeline.run_item_research_agent", side_effect=fake_agent),
        patch("schnapplist.agents.process_pipeline.write_item_report"),
    ):
...
def test_pipeline_item_done_includes_low_confidence_false(tmp_path):
    ...
    with (
        ...
        patch("schnapplist.agents.process_pipeline.filter_redundant_photos", return_value=[photos_dir / "item.jpg"]),
        patch("schnapplist.agents.process_pipeline.enhance_photo", return_value=photos_dir / "item.jpg"),
        patch("schnapplist.agents.process_pipeline.run_item_research_agent", return_value=MagicMock(output=mock_output, usage=MagicMock())),
        patch("schnapplist.agents.process_pipeline.write_item_report"),
    ):
